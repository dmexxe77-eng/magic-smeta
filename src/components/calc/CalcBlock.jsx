...
function MultiBlock({config,list,setList,presets,onPresets,favIds,setFavIds,onEditNom,nomSnap,onOpenEditor,roomInfo}){const add=()=>{setList(p=>[...p,{id:uid(),btnId:"",qty:0,off:{},oq:{}}]);};return(<div style={{marginBottom:8}}>{list.map((inst,i)=>(<div key={inst.id} style={{position:"relative"}}><span onClick={()=>setList(p=>{const n=[...p];n.splice(i,1);return n;})} style={{position:"absolute",top:4,right:36,color:T.red,cursor:"pointer",fontSize:13,zIndex:2,padding:4,background:T.card,borderRadius:6}}>{"×"}</span><CalcBlock config={config} instance={inst} favIds={favIds} setFavIds={setFavIds} onChange={v=>setList(p=>{const n=[...p];n[i]=v;return n;})} presets={presets} onPresets={onPresets} onEditNom={onEditNom} nomSnap={nomSnap} onOpenEditor={onOpenEditor} roomInfo={roomInfo}/></div>))}<div onClick={add} style={{textAlign:"center",padding:"9px 6px",color:T.accent,fontSize:11.5,fontWeight:700,cursor:"pointer",background:"rgba(79,70,229,0.05)",border:"1.5px dashed rgba(79,70,229,0.4)",borderRadius:10}}>{"+ "+config.title}</div></div>);}

function ExtraBlock({list,setList,onEditNom,nomSnap}){
  const[showAdd,setShowAdd]=useState(false);
  const gp=n=>(nomSnap&&nomSnap[n.id]!==undefined)?nomSnap[n.id]:n.price;
  return(<div style={{background:T.card,borderRadius:12,padding:10,marginBottom:8}}>
    <div style={{fontSize:10,fontWeight:600,color:T.accent,textTransform:"uppercase",letterSpacing:"0.4px",marginBottom:6}}>{"Доп. работы и материалы"}</div>
    {list.map((item,i)=>{const n=NB(item.nomId);if(!n)return null;
      return(<div key={item.id} style={{display:"flex",alignItems:"center",gap:4,padding:"4px 0",borderBottom:"0.5px solid "+T.border}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:11,color:T.text}}>{n.name}</div>
          <div style={{fontSize:9,color:T.dim}}>{fmt(gp(n))+" /"+n.unit}</div>
        </div>
        <NI value={item.qty||0} onChange={v=>setList(p=>{const c=[...p];c[i]={...c[i],qty:v};return c;})} w={32}/>
        <span style={{fontSize:11,fontWeight:500,color:T.accent,minWidth:40,textAlign:"right"}}>{fmt((item.qty||0)*gp(n))}</span>
        <span onClick={()=>onEditNom?.(item.nomId)} style={{color:T.accent,cursor:"pointer",fontSize:12,padding:2}}>{"\u270e"}</span>
        <span onClick={()=>setList(p=>{const c=[...p];c.splice(i,1);return c;})} style={{color:T.red,cursor:"pointer",fontSize:13,padding:2}}>{"\u00d7"}</span>
      </div>);})}
    <div onClick={()=>setShowAdd(true)} style={{textAlign:"center",padding:"9px 6px",color:T.accent,fontSize:11.5,fontWeight:700,cursor:"pointer",background:"rgba(79,70,229,0.05)",border:"1.5px dashed rgba(79,70,229,0.4)",borderRadius:10,marginTop:6}}>{"+ Из номенклатур"}</div>
    {/* Общий пикер базы — как в редакторах кнопок и номенклатур */}
    {showAdd&&<NomPicker title="Доп. работы и материалы" onPick={ids=>{setList(p=>[...p,...ids.map(id=>({id:uid(),nomId:id,qty:1}))]);setShowAdd(false);}} onClose={()=>setShowAdd(false)}/>}
  </div>);}
