...
function CalcBlock({config,instance,onChange,presets,onPresets,autoAngles,onApplyAll,favIds,setFavIds,onEditNom,nomSnap}){
  ...
  return(<div style={{background:T.card,borderRadius:12,padding:10,marginBottom:8}}>
    ...
    {pr&&(()=>{
      ...
      const rows=[
        ...items.map(n=>({n,src:"param"})),
        ...opts.map(n=>({n,src:legacyOptionSrc(n.id)})),
      ];
      return(<div>
        ...
        {rows.map(({n,src})=>{
          ...
          const baseQ=(ocArea&&n.type==="canvas")?ocArea:effectiveQ;
          const iq=instance.iq?.[n.id];
          const qUse=isParam?(iq!=null?iq:baseQ):(instance.oq?.[n.id]||0);
...
function MultiBlock({config,list,setList,presets,onPresets,favIds,setFavIds,onEditNom,nomSnap}){const add=()=>{const f=presets.filter(p=>p.cat===config.cat);setList(p=>[...p,{id:uid(),btnId:f[0]?.id||"",qty:0,off:{},oq:{}}]);};return(<div style={{marginBottom:8}}>{list.map((inst,i)=>(<div key={inst.id} style={{position:"relative"}}><span onClick={()=>setList(p=>{const n=[...p];n.splice(i,1);return n;})} style={{position:"absolute",top:4,right:36,color:T.red,cursor:"pointer",fontSize:13,zIndex:2,padding:4,background:T.card,borderRadius:6}}>{"×"}</span><CalcBlock config={config} instance={inst} favIds={favIds} setFavIds={setFavIds} onChange={v=>setList(p=>{const n=[...p];n[i]=v;return n;})} presets={presets} onPresets={onPresets} onEditNom={onEditNom} nomSnap={nomSnap}/></div>))}<div onClick={add} style={{textAlign:"center",padding:6,color:"#9aa0b2",fontSize:10.5,fontWeight:700,cursor:"pointer",background:"transparent"}}>{"+ "+config.title}</div></div>);}
