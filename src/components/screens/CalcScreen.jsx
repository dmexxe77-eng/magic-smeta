...
function CalcScreen({onMenu,initRooms,orderName,onBack,onRoomsChange,initPlanImage,initMode,onPlanImageChange,onSnapshotUpdate,initNomSnapshot,initOptsOn,onOptsChange}){
  ...
  const applyButtonEditor=(newPresets,favs,blocks)=>{
    ...
    setRooms(prev=>{
      let changed=false;
      const fix=(inst,cat)=>{
        if(!inst||!inst.btnId)return inst;
        const vis=favs[cat]||[];
        if(!vis.length||vis.includes(inst.btnId))return inst;
  ...
  /* данные текущего помещения для автоподстановки источников (S / P / углы) */
  const roomInfo={area:gA(r),perim:gP(r),cIn:inn,cOut:out};
  ...
  return(<div style={{minHeight:"100vh",background:"#f2f3fa",color:"#1e2530",fontFamily:"'Inter',-apple-system,system-ui,sans-serif",fontSize:12}}>
    ...
    <div className="mw-wide" style={{padding:10}}>
      {/* Room tabs */}
      ...
      <div style={{display:"flex",gap:6,marginBottom:6}}>
        <div style={{flex:1}} onClick={()=>setPolyEdit(true)}>
          <PolyMini verts={r.v} areaOverride={r.aO} perimOverride={r.pO} roll={(()=>{
            /* Отрез полотна — только при включённом перерасходе */
            if(!r.canvas?.overcut)return null;
            const cPr=presets.find(p=>p.id===r.canvas?.btnId);if(!cPr)return null;
            const cNom=(cPr.items||[]).map(id=>NB(id)).find(n=>n?.type==="canvas");
            const cp=cutParams(cPr,r.canvas);
            const rbox=roomBox(r.v,cp.marginM);
            const ws=widthList(cNom,activeNoms());
            if(!rbox||!ws)return null;
            const auto=bestWidth(rbox,ws,cp.shrink,r.canvas?.wDir);
            const wEff=r.canvas?.wSel||auto?.w;if(!wEff)return null;
            const use=canvasUsage(rbox,wEff,cp.shrink,r.canvas?.wDir);if(!use)return null;
            const axis=(use.dir==="a")===(rbox.ax==="x")?"y":"x";
            return{cm:Math.round(wEff*100),wM:wEff,mM:cp.marginM,k:1-(cp.shrink||0)/100,strips:use.strips,axis};
          })()}/>
        </div>
      </div>
      </div>
      ...
      {/* Blocks */}
      <CalcBlock config={BLOCK_CFG[0]} favIds={sharedFavs["canvas"]} setFavIds={ids=>setSharedFavs(p=>({...p,"canvas":ids}))} instance={{...(r.canvas||{}),verts:r.v,qty:gA(r),_geo:Math.round(poly.a*100)/100}} onChange={v=>{const{verts,_geo,...rest}=v;const cleaned={...rest,iq:{}};u(r.id,rm=>{rm.canvas=cleaned;
        /* S в блоке — переопределение площади помещения (как раньше в редакторе контура): его видят смета, экспорт и подписи.
           0 или значение из чертежа снимают переопределение */
        const q=Number(cleaned.qty),gq=Math.round(calcPoly(rm.v||[]).a*100)/100;if(q!==gA(rm))rm.aO=(isFinite(q)&&q>0&&Math.abs(q-gq)>0.004)?q:null;return rm;});if(cleaned.applyAll){rooms.forEach(rm2=>{if(rm2.id===r.id)return;const a2=gA(rm2);u(rm2.id,rm3=>{rm3.canvas={...cleaned,id:rm3.canvas?.id||uid(),qty:a2,iq:{}};return rm3;});});}}} presets={presets} onPresets={setPresets} autoAngles={autoAngles} onApplyAll={()=>{const cv={...(r.canvas||{}),iq:{}};rooms.forEach(rm2=>{if(rm2.id===r.id)return;const a2=gA(rm2);u(rm2.id,rm3=>{rm3.canvas={...JSON.parse(JSON.stringify(cv)),id:rm3.canvas?.id||uid(),qty:a2,iq:{}};return rm3;});});}} onEditNom={openNomEditorFromCalc} onOpenEditor={openButtonEditor} roomInfo={roomInfo} nomSnap={nomSnapshot}/>
      {/* Доп. полотна */}
      ...
      <div onClick={()=>u(r.id,rm=>{if(!rm.extraCanvas)rm.extraCanvas=[];rm.extraCanvas.push({id:uid(),btnId:"",qty:0,off:{},oq:{}});return rm;})} style={{textAlign:"center",padding:"9px 6px",color:T.accent,fontSize:11.5,fontWeight:700,cursor:"pointer",background:"rgba(79,70,229,0.05)",border:"1.5px dashed rgba(79,70,229,0.4)",borderRadius:10,marginBottom:8}}>{"+ Доп. полотно"}</div>
      <CalcBlock config={BLOCK_CFG[1]} favIds={sharedFavs["main"]} setFavIds={ids=>setSharedFavs(p=>({...p,"main":ids}))} instance={{...(r.mainProf||{}),qty:gP(r),_geo:Math.round(poly.p*100)/100,_subTotal:(r.extras||[]).filter(x=>x.subP).reduce((s,x)=>s+(x.qty||0),0)+(r.curtains||[]).filter(x=>x.subP).reduce((s,x)=>s+(x.qty||0),0)}} onChange={v=>{const{_geo,_subTotal,...mp}=v;u(r.id,rm=>{rm.mainProf=mp;
        /* P в блоке — переопределение периметра помещения: раньше смета считала профиль только по чертежу и введённое число игнорировала */
        const q=Number(mp.qty),gq=Math.round(calcPoly(rm.v||[]).p*100)/100;if(q!==gP(rm))rm.pO=(isFinite(q)&&q>0&&Math.abs(q-gq)>0.004)?q:null;return rm;});if(mp.applyAll){rooms.forEach(rm2=>{if(rm2.id===r.id)return;const p2=gP(rm2);const angs2=getAngles((rm2.v||[]).map(pp=>[pp[0]*1000,pp[1]*1000]));const inn2=angs2.filter(d=>d===90).length,out2=angs2.filter(d=>d===270).length;u(rm2.id,rm3=>{rm3.mainProf={...JSON.parse(JSON.stringify(mp)),id:rm3.mainProf?.id||uid(),qty:p2,oq:{...mp.oq,"o_inner_angle":inn2,"o_outer_angle":out2,"o_angle":inn2+out2}};return rm3;});});}}} presets={presets} onPresets={setPresets} autoAngles={autoAngles} onApplyAll={()=>{const mp=r.mainProf||{};rooms.forEach(rm2=>{if(rm2.id===r.id)return;const p2=gP(rm2);const angs2=getAngles((rm2.v||[]).map(pp=>[pp[0]*1000,pp[1]*1000]));const inn2=angs2.filter(d=>d===90).length,out2=angs2.filter(d=>d===270).length;u(rm2.id,rm3=>{rm3.mainProf={...JSON.parse(JSON.stringify(mp)),id:rm3.mainProf?.id||uid(),qty:p2,oq:{...mp.oq,"o_inner_angle":inn2,"o_outer_angle":out2,"o_angle":inn2+out2}};return rm3;});});}} onEditNom={openNomEditorFromCalc} onOpenEditor={openButtonEditor} roomInfo={roomInfo} nomSnap={nomSnapshot}/>
      ...
      <MultiBlock config={BLOCK_CFG[4]} favIds={sharedFavs["track"]} setFavIds={ids=>setSharedFavs(p=>({...p,"track":ids}))} list={r.tracks||[]} setList={v=>{const fn=typeof v==="function"?v:()=>v;u(r.id,rm=>{rm.tracks=fn(rm.tracks||[]);return rm;});}} presets={presets} onPresets={setPresets} onEditNom={openNomEditorFromCalc} onOpenEditor={openButtonEditor} roomInfo={roomInfo} nomSnap={nomSnapshot}/>
      <MultiBlock config={BLOCK_CFG[5]} favIds={sharedFavs["curtain"]} setFavIds={ids=>setSharedFavs(p=>({...p,"curtain":ids}))} list={r.curtains||[]} setList={v=>{const fn=typeof v==="function"?v:()=>v;u(r.id,rm=>{rm.curtains=fn(rm.curtains||[]);return rm;});}} presets={presets} onPresets={setPresets} onEditNom={openNomEditorFromCalc} onOpenEditor={openButtonEditor} roomInfo={roomInfo} nomSnap={nomSnapshot}/>
      {/* Свои блоки из редактора кнопок */}
      {customBlocks.map(b=>(<MultiBlock key={b.id} config={{id:b.id,cat:b.id,title:b.label,qtyLabel:"Кол",qtyUnit:"шт",multi:true,maxFav:99}} favIds={sharedFavs[b.id]} setFavIds={ids=>setSharedFavs(p=>({...p,[b.id]:ids}))} list={(r.cst||{})[b.id]||[]} setList={v=>{const fn=typeof v==="function"?v:()=>v;u(r.id,rm=>{rm.cst={...(rm.cst||{}),[b.id]:fn((rm.cst||{})[b.id]||[])};return rm;});}} presets={presets} onPresets={setPresets} onEditNom={openNomEditorFromCalc} onOpenEditor={openButtonEditor} roomInfo={roomInfo} nomSnap={nomSnapshot}/>))}
      <ExtraBlock list={r.extraItems||[]} setList={v=>{const fn=typeof v==="function"?v:()=>v;u(r.id,rm=>{rm.extraItems=fn(rm.extraItems||[]);return rm;});}} onEditNom={openNomEditorFromCalc} onOpenEditor={openButtonEditor} roomInfo={roomInfo} nomSnap={nomSnapshot}/>
...
@media print{.room-header,thead tr{print-color-adjust:exact;-webkit-print-color-adjust:exact}body{background:#fff}.page{box-shadow:none}}`;
            ...
            const rowHtml=(l,i,photoDataUrl)=>{
              const nameCell=photoDataUrl
                ...
                :String(l.n||"");
              const eq=estEd[l.k]?.q??l.q,ep=estEd[l.k]?.p??l.p; /* правки строк из панели «Смета» — как в итогах */
              return '<tr>'+cD(C.num,String(i+1))+cD(C.name,nameCell)+cD(C.qty,fmt(eq))+cD(C.unit,String(l.u||""))+cD(C.price,fmt(ep),'r')+cD(C.total,fmtRub(eq*ep),'r total-val')+'</tr>';
            };
