...
/* Договор для страницы дизайна: предпросмотр → PDF / поделиться / сохранить в проект, редактор шаблона.
   Шаблон один на студию, хранится в localStorage. Токены — design-src/README.md, раздел 1. */
const D = { ink: '#1E2530', sub: '#6F7688', dim: '#A5A9B8', accent: '#4F46E5', soft: '#EEEDFC', bg: '#F3F3FA', card: '#fff', field: '#F7F7FC', line: '#ECECF4', border: '#E4E4EE', success: '#16A34A', successSoft: '#E6F6EC', danger: '#FF3B30', dangerSoft: '#FFECEA' };
...
function ContractApp({ project, total, area, est, contract, installDate: ins0, onSaveContract, onProjectPatch, onClose }) {
  ...
  const insertPh = ph => {
    ...
    const text = sec.text.slice(0, a) + '{' + ph + '}' + sec.text.slice(b);
    patchSec(sec.id, { text });
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9000, background: D.bg, overflowY: 'auto', fontFamily: "'Manrope','Inter',system-ui,sans-serif", color: D.ink, WebkitFontSmoothing: 'antialiased' }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 5, background: D.card, borderBottom: '1px solid ' + D.line, padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11.5, fontWeight: 600, color: D.sub, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{project.name}</div>
        </div>
        <div style={{ display: 'flex', background: D.bg, borderRadius: 12, padding: 3, gap: 3, flexShrink: 0 }}>
          {[['preview', 'Просмотр'], ['template', 'Шаблон']].map(([id, l]) => (
            <button key={id} onClick={() => setMode(id)} style={{ height: 30, padding: '0 12px', borderRadius: 9, border: 'none', background: mode === id ? D.card : 'transparent', color: mode === id ? D.ink : D.sub, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: mode === id ? '0 1px 3px rgba(30,37,48,.12)' : 'none' }}>{l}</button>))}
        </div>
      </div>

      <div style={{ maxWidth: 720, margin: '0 auto', padding: '12px 12px calc(40px + env(safe-area-inset-bottom))' }}>
        {mode === 'preview' && (<>
          <div style={S.card}>
        ...
        {mode === 'template' && (<>
          <div style={S.card}>
            <div style={S.sec}>Шапка исполнителя</div>
          ...
          <div style={S.card}>
            ...
            <div style={{ fontSize: 12, color: D.sub, fontWeight: 600, marginBottom: 8, lineHeight: 1.4 }}>Поставьте курсор в текст раздела и нажмите нужную подстановку. При печати она заменится данными проекта.</div>
...
export function open(opts) {
  ...
  root.render(<ContractApp {...opts} onClose={() => { if (opts.onClose) opts.onClose(); else close(); }} />);
  return { close };
}
