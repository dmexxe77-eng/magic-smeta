...
export function makeModel(pts, mode) { const n = pts.length, wind = windOf(pts);
  // режим косых стен: углы по рисунку не угадываем, все свободные — строим по длинам, диагоналям и градусам
  const vert = pts.map((p, i) => { if (mode === 'free') return { kind: 'free', deg: null }; const d = interiorDeg(pts, i, wind); if (Math.abs(d - 90) <= 4) return { kind: 'ortho', deg: 90 }; if (Math.abs(d - 270) <= 4) return { kind: 'ortho', deg: 270 }; if (Math.abs(d - 180) <= 4) return { kind: 'deg', deg: 180 }; return { kind: 'free', deg: null }; });
...
export function polyStats(poly) { const F = flatten(poly), P = ptsOf(poly), wind = windOf(P); let inner = 0, outer = 0;
  ...
  let area = Math.abs(shoelace(F)), perim = perimeterOf(F), fillets = poly.v.filter(p => p.fillet).length, arcs = poly.v.filter(p => p.arc).length;
  /* вырезы: площадь уходит, профиль идёт и по их стенам; прямой угол выреза для полотна — наружный */
  (poly.holes || []).forEach(h => { const HF = flatten(h), HP = ptsOf(h), hw = windOf(HP);
    HP.forEach((p, i) => { if (interiorDeg(HP, i, hw) < 180) outer++; else inner++; });
    area -= Math.abs(shoelace(HF)); perim += perimeterOf(HF); fillets += h.v.filter(p => p.fillet).length; arcs += h.v.filter(p => p.arc).length; });
  return { area, perim, inner, outer, fillets, arcs, holes: (poly.holes || []).length }; }
...
// ───────── внутренние вырезы: poly.holes = [{ v: [...] }], каждый — такой же контур со своими углами и стенами ─────────
export const holesOf = poly => poly.holes || [];
export const flattenHoles = (poly, stepDeg) => holesOf(poly).map(h => flatten(h, stepDeg));
/* Габарит контура: от левой и верхней стены отсчитываются отступы выреза */
export function bboxOf(P) { let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
  P.forEach(p => { x0 = Math.min(x0, p.x); y0 = Math.min(y0, p.y); x1 = Math.max(x1, p.x); y1 = Math.max(y1, p.y); }); return { x0, y0, x1, y1, w: x1 - x0, h: y1 - y0 }; }
const segsX = (A, B) => { for (let i = 0; i < A.length; i++) for (let j = 0; j < B.length; j++) if (segX(A[i], A[(i + 1) % A.length], B[j], B[(j + 1) % B.length])) return true; return false; };
/* Вырез должен целиком лежать внутри потолка и не задевать другие вырезы; иначе — текст ошибки */
export function holeError(poly, k) { const outer = flatten(poly), hs = flattenHoles(poly), H = hs[k];
  if (Math.abs(shoelace(H)) < 1e-6) return 'Вырез вырождается';
  if (H.some(p => !inside(outer, p)) || segsX(H, outer)) return 'Вырез выходит за стены помещения';
...
export function withHole(poly, k, fn) { const q = clone(poly), hs = holesOf(q); if (!hs[k]) throw new Error('Вырез не найден');
  hs[k] = fn(hs[k]); q.holes = hs; const e = holeError(q, k); if (e) throw new Error(e); return q; }
